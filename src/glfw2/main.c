    ...
    while ((opt = getopt_long(argc, argv, "", longOptions, nullptr)) != -1) {
        switch (opt) {
            ...
            case 'O':
                ...
                break;
            default:
                fprintf(stderr, "Usage: %s [--screenshot=PATTERN] [--screenshot-at-frame=N ...] <path to data.win or game.unx>\n", argv[0]);
                exit(1);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    while (glfwGetWindowParam(GLFW_OPENED) && !runner->shouldExit) {
        ...
        if (runner->currentRoom->speed > 0) {
            ...
            bool fastForwardTabNow = glfwGetKey(GLFW_KEY_TAB) == GLFW_PRESS;
