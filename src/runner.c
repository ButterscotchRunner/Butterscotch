...
static void initRoom(Runner* runner, int32_t roomIndex) {
    ...
    // Fix: Force immediate transition from rm_snowdin_19_yellow to rm_snowdin_20_yellow
    if (strcmp(room->name, "rm_snowdin_19_yellow") == 0) {
        // Find rm_snowdin_20_yellow index
        for (uint32_t i = 0; i < runner->dataWin->room.count; i++) {
            if (strcmp(runner->dataWin->room.rooms[i].name, "rm_snowdin_20_yellow") == 0) {
                fprintf(stderr, "Forcing immediate transition from rm_snowdin_19_yellow to rm_snowdin_20_yellow\n");
...
void Runner_handlePendingRoomChange(Runner* runner) {
    ...
    if (runner->pendingRoom >= 0) {
        ...
        // Fix: Force transition from rm_snowdin_19_yellow to rm_snowdin_20_yellow
        if (strcmp(oldRoomName, "rm_snowdin_19_yellow") == 0 && strcmp(runner->dataWin->room.rooms[newRoomIndex].name, "rm_snowdin_20_yellow") == 0) {
            fprintf(stderr, "Forced room transition from rm_snowdin_19_yellow to rm_snowdin_20_yellow\n");
            // Continue with the transition (don't block it)
        }
...
int32_t Runner_runtimeTimelineAdd(Runner* runner, const char* name) {
    ...
    int32_t timelineId = (int32_t) runner->dataWin->tmln.count + (int32_t) arrlen(runner->runtimeTimelines) - 1;
    fprintf(stderr, "Timeline: Created runtime timeline '%s' with ID %d\n", name, timelineId);
    return timelineId;
}
...
bool Runner_runtimeTimelineAddMomentScript(Runner* runner, int32_t timelineIndex, int32_t step, int32_t codeId) {
    ...
    int32_t runtimeIndex = timelineIndex - (int32_t) tmln->count;
    if (runtimeIndex < 0 || (int32_t) arrlen(runner->runtimeTimelines) <= runtimeIndex) {
        fprintf(stderr, "Timeline: Failed to add moment to timeline %d (invalid runtime index %d)\n", timelineIndex, runtimeIndex);
        return false;
    }

    RuntimeTimeline* rt = &runner->runtimeTimelines[runtimeIndex];
    fprintf(stderr, "Timeline: Adding moment to timeline %d at step %d with codeId %d\n", timelineIndex, step, codeId);
...
static void tickTimelines(Runner* runner) {
    ...
    repeat(n, i) {
        ...
        if (timeline == nullptr || timeline->momentCount == 0) {
            // Log if instance has invalid timeline
            static int32_t lastLoggedInvalidId = -1;
            if (inst->instanceId != lastLoggedInvalidId) {
                fprintf(stderr, "Timeline: Instance %d (object %d) has invalid timeline %d (nullptr=%d, momentCount=%d)\n", inst->instanceId, inst->objectIndex, idx, timeline == nullptr, timeline ? timeline->momentCount : 0);
                lastLoggedInvalidId = inst->instanceId;
            }
            continue;
        }

        if (!inst->timelineRunning) {
            // Log if instance has timeline assigned but it's not running
            static int32_t lastLoggedInstanceId = -1;
            if (inst->instanceId != lastLoggedInstanceId) {
                fprintf(stderr, "Timeline: Instance %d (object %d) has timeline %d assigned but not running\n", inst->instanceId, inst->objectIndex, idx);
                lastLoggedInstanceId = inst->instanceId;
            }
            continue;
        }
