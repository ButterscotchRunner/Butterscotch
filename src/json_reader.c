    ...
    while (parser->position < parser->length) {
        ...
        if (c == '\\') {
            ...
            switch (escaped) {
                ...
                default:
                    Log_logWarning("JsonReader: unknown escape sequence '\\%c'\n", escaped);
                    free(buffer);
                    return nullptr;
            }
        }
...
static JsonValue* parseValue(JsonParser* parser) {
    ...
    switch (c) {
        ...
        default:
            ...
            Log_logWarning("JsonReader: unexpected character '%c' at position %zu\n", c, parser->position);
            return nullptr;
    }
}
...
JsonValue* JsonReader_parse(const char* json) {
    ...
    if (result != nullptr) {
        ...
        if (parser.position < parser.length) {
            Log_logWarning("JsonReader: trailing content after JSON value at position %zu\n", parser.position);
