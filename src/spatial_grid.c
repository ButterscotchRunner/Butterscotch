...
SpatialGridQuery SpatialGrid_prepareQuery(Runner* runner, GMLReal x1, GMLReal y1, GMLReal x2, GMLReal y2, int32_t target) {
    ...
    requireMessageFormatted(target >= 0 || target == INSTANCE_ALL, __FILE__, __LINE__, "SpatialGrid: [%s] Query target cannot be instance type %d!", runner->vmContext->currentCodeName, target);
    ...
    {
        SpatialGridQuery _q;
        _q.range = callerRange;
        _q.filterByObject = filterByObject;
        _q.filterByInstanceId = filterByInstanceId;
        _q.matchAll = (target == INSTANCE_ALL);
        _q.queryId = queryId;
        return _q;
    }
}
