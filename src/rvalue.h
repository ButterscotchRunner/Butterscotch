...
static inline RValue RValue_makeIndependent(RValue val) {
    ...
    requireMessageFormatted(!val.ownsReference, __FILE__, __LINE__, "Trying to make independent a RValue (type=%d) that owns a reference, but we don't handle it yet! Did you add a new refcounted value to Butterscotch without implementing RValue_makeIndependent for it?", val.type);
    return val;
}
