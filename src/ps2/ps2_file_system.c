...
static void copyIconIcoIfMissing(const char* dirPath) {
    ...
    if (copyFile(srcPath, dstPath)) {
        Log_log("Ps2FileSystem: Copied ICON.ICO to %s\n", dirPath);
    } else {
        Log_logWarning("Ps2FileSystem: Failed to copy ICON.ICO from %s to %s\n", srcPath, dstPath);
    }
...
static void writeIconSysIfMissing(const char* dirPath, const char* gameTitle, const SaveIconConfig* config) {
    ...
    if (f != nullptr) {
        ...
        fclose(f);
        Log_log("Ps2FileSystem: Created icon.sys in %s\n", dirPath);
    } else {
        Log_logWarning("Ps2FileSystem: Failed to create icon.sys in %s\n", dirPath);
    }
