...
static void parseSoundBank(Ps2AudioSystem* ps2) {
    ...
    Log_log("PS2AudioSystem: SOUNDBNK v%d, %d SOND entries, %d AUDO entries, %d MUS entries\n", version, ps2->sondEntryCount, ps2->audoEntryCount, ps2->musEntryCount);
    ...
    if (ps2->musEntryCount > 0) {
        Log_log("PS2AudioSystem: Loaded %d MUS entries\n", ps2->musEntryCount);
    }
...
static void openSoundsBin(Ps2AudioSystem* ps2) {
    ...
    if (ps2->soundsFile == nullptr) {
        Log_logWarning("PS2AudioSystem: Could not open SOUNDS.BIN at %s\n", path);
        free(path);
        return;
    }

    Log_log("PS2AudioSystem: Opened SOUNDS.BIN for streaming (%s)\n", path);
    free(path);
}
...
static DecodedPcmEntry* cacheInsert(Ps2AudioSystem* ps2, int32_t audoIndex) {
    ...
    if (slot == nullptr) {
        ...
        if (slot == nullptr) {
            // Log_logWarning("PS2AudioSystem: Cache full, all entries in use! Cannot decode audoIndex %" PRId32 "\n", audoIndex);
            return nullptr;
        }
...
static void ps2Init(AudioSystem* audio, MAYBE_UNUSED DataWin* dataWin, MAYBE_UNUSED FileSystem* fileSystem) {
    ...
    int ret = audsrv_init();
    if (ret != 0) {
        Log_logWarning("PS2AudioSystem: audsrv_init failed (%d)\n", ret);
        return;
    }
    ...
    if (ret != 0) {
        Log_logWarning("PS2AudioSystem: audsrv_set_format failed (%d)\n", ret);
        audsrv_quit();
        return;
    }
    ...
    Log_log("PS2AudioSystem: Initialized (output: %d Hz, 16-bit, stereo)\n", AUDSRV_OUTPUT_FREQ);
}
...
static void ps2Update(AudioSystem* audio, float deltaTime) {
    ...
    repeat(MAX_MUSIC_STREAMS, i) {
        ...
        // Log_log("PS2AudioSystem: Filling music stream %d back buffers...\n", stream->soundIndex);
    ...
    while (audsrv_available() >= chunkBytes) {
        // Log_log("PS2AudioSystem: Filling audsrv ring buffer... audsrv_available: %d, chunkBytes: %d\n", audsrv_available(), chunkBytes);
...
static int32_t ps2PlaySound(AudioSystem* audio, int32_t soundIndex, int32_t priority, bool loop) {
    // Log_log("PS2AudioSystem: Attempting to play sound index %d with priority %d, should loop? %d\n", soundIndex, priority, loop);
    Ps2AudioSystem* ps2 = (Ps2AudioSystem*) audio;
    ...
    if (soundIndex >= PS2_AUDIO_STREAM_INDEX_BASE) {
        ...
        // Log_log("PS2AudioSystem: Streaming MUS '%s', size=%" PRIu32 " bytes, instanceId=%" PRId32 "\n", mus->name, mus->dataSize, instanceId);

        return instanceId;
    }

    if (0 > soundIndex || (uint16_t) soundIndex >= ps2->sondEntryCount) {
        // Log_logWarning("PS2AudioSystem: Invalid sound index %" PRId32 "\n", soundIndex);
        return -1;
    }
    ...
    if (sond->audoIndex >= ps2->audoEntryCount) {
        // Log_logWarning("PS2AudioSystem: Invalid audo index %d for sound %" PRId32 "\n", sond->audoIndex, soundIndex);
        return -1;
    }
    ...
    if (!isEmbedded && !isCompressed) {
        ...
        // Log_log("PS2AudioSystem: Streaming music soundIndex=%" PRId32 " audoIndex=%d, size=%" PRIu32 " bytes, instanceId=%" PRId32 "\n", soundIndex, sond->audoIndex, audo->dataSize, instanceId);
    ...
    if (cached == nullptr) {
        ...
        if (cached == nullptr) {
            // Log_logWarning("PS2AudioSystem: Failed to cache decoded audio for sound %" PRId32 "\n", soundIndex);
            return -1;
        }
    }
    ...
    if (slot == nullptr) {
        // Log_logWarning("PS2AudioSystem: No free sound slots for sound %" PRId32 "\n", soundIndex);
        return -1;
    }
...
static void ps2StopSound(AudioSystem* audio, int32_t soundOrInstance) {
    // Log_log("PS2AudioSystem: Stopping sound %d\n", soundOrInstance);
...
static void ps2PauseSound(AudioSystem* audio, int32_t soundOrInstance) {
    // Log_log("PS2AudioSystem: Pausing sound %d\n", soundOrInstance);
...
static void ps2ResumeSound(AudioSystem* audio, int32_t soundOrInstance) {
    // Log_log("PS2AudioSystem: Resuming sound %d\n", soundOrInstance);
...
static void ps2SetSoundPitch(AudioSystem* audio, int32_t soundOrInstance, float pitch) {
    // Log_log("PS2AudioSystem: Setting pitch of sound %d to %f\n", soundOrInstance, pitch);
...
static void ps2SetTrackPosition(AudioSystem* audio, int32_t soundOrInstance, float positionSeconds) {
    // Log_log("PS2AudioSystem: Setting track position of sound %d to %f\n", soundOrInstance, positionSeconds);
    Ps2AudioSystem* ps2 = (Ps2AudioSystem*) audio;
...
static void ps2SetMasterGain(AudioSystem* audio, float gain) {
    // Log_log("PS2AudioSystem: Setting master gain to %f\n", gain);
    Ps2AudioSystem* ps2 = (Ps2AudioSystem*) audio;
    ps2->masterGain = gain;
}
...
static int32_t ps2CreateStream(AudioSystem* audio, const char* filename) {
    ...
    for (int i = 0; ps2->musEntryCount > i; i++) {
        if (strcmp(ps2->musEntries[i].name, filename) == 0) {
            ...
            Log_log("PS2AudioSystem: Created stream %" PRId32 " for '%s'\n", streamIndex, filename);
            return streamIndex;
        }
    }

    Log_logWarning("PS2AudioSystem: audio_create_stream: '%s' not found in MUS entries\n", filename);
    return -1;
}
